...
- `a` / `콘텐츠 활동을 본격화한다.`
  ...
  - `result`: `multi([trait_delta(domain='per', attribute='interpersonal', traitType='active', delta=1), asset_delta(amount=20000000)])`
  - `asset_delta(amount=20000000)`으로 자산이 `2000만원` 증가한다.
- `b` / `관심이 부담되어 숨는다.`
  - `resultText`: `잠깐의 바이럴로 광고 수익이 조금 들어왔습니다.`
  - `result`: `multi([trait_delta(domain='per', attribute='interpersonal', traitType='active', delta=-1), asset_delta(amount=5000000)])`
